...
export interface LedContext {
  ...
  // threat (square control + intensity) and defense (piece defenders) overlays
  // derive from the ENGINE — the source of truth — instead of the chess.js relation
  // map. Falls back to chess.js when absent (old binary / facts pending / unavailable).
  // Hanging stays chess.js: Rust SEE is computed only for the side-to-move's targets.
...
function threatMode(ctx: LedContext): LedMap {
  ...
  map.intensity = {};
  // Engine as source of truth (PR-16): per-square controller COUNTS from Rust
  // squareFacts drive both the colour and the intensity (opacity) when computed;
  // otherwise fall back to chess.js attacker counts (same count semantics).
  const squareFacts = ctx.enginePosition?.squareFacts;
  if (squareFacts && squareFacts.status === 'computed') {
    const bySquare = new Map(squareFacts.items.map((s) => [s.square, s]));
    for (const sq of allSquares()) {
      const sf = bySquare.get(sq);
      const wn = sf?.attackedByWhite.length ?? 0;
      const bn = sf?.attackedByBlack.length ?? 0;
      if (wn && bn) {
