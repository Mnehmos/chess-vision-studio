...
## NNUE calibration + BUG1 scoping — 2026-06-28
- **Calibration gate**: 219k-d20 net vs gen8v2 champion (matrix-raw) = champ **82-3-15, +372 Elo (LOS 100%)**. NNUE is DATA-limited: deeper d20 labels don't beat 24x more d12 data at this size. Full pipeline (join→train→load→cutechess-gate) validated end-to-end. Strategy: relabel continues toward 1M for the *definitive* large-corpus gate (5x less data than champion's 5.34M — uncertain but worth the definitive test) + reusable for a d12+d20 combine; ACTIVE work pivots to the reliable search lever (lmp+smarttime precedent).
- **Train pipeline notes**: `join-d20-res.py` (scratchpad) joins d20 labels (sfCp/sfMate, res=null) with selfplay `res` by fen → train-nnue.py reads cp via `j.get('cp', j.get('sfCp'))` + sfMate→±10000. Training is FAST (~0s/epoch for 220k on the 4070) — relabel is the sole bottleneck (~33k/hr d20).
- **BUG1 mate-TT — SCOPED, implement next**: search uses root-relative mate scores `±(MATE_SCORE - (ply+d))` (MATE_SCORE=1e6, MATE_THRESHOLD=1e6-1000). Make TT entries node-intrinsic. STORE: +ply if >MATE_THRESHOLD, -ply if <-MATE_THRESHOLD. PROBE: -ply / +ply resp. Sites: `store` wrapper search.rs:470 (add `ply` param, gate on `opts.matett`) called from qsearch.rs:75,142 + root.rs:108,217,309,579,590 (all have ply); probe at root.rs:158 (`if let Some(mut e)=self.tt_probe(...)` → adjust `e.score`). Add `--matett` flag to SearchOptions (types.rs, mirror `--lmp`). Unit test: store mate at ply A, probe at ply B, assert mate distance preserved. Gate: uci.exe --matett vs uci.exe ~300g.

## BUG1 mate-TT GATE — 2026-06-28: NEUTRAL, NOT promoted
- base vs `--matett`, 300g, champion net (matrix-raw), tc=10+0.1, conc6: base **99-97-104, Elo +2.3 ± 31.8, LOS 55.7%** (CI includes 0) → WASH. Keep-gate REJECTS. Code kept flag-gated OFF (engine commit 244ad59; correct mate-TT but Elo-neutral — mate scores too rare in play). 4 unit tests pass (mate distance preserved across plies).
- Session search-heuristic tally: lmp+smarttime +111 (PROMOTED), BUG1 ~0 (rejected). Estimates run optimistic; gate everything.
- NEXT: log-based LMR (handoff #4, +20-40 est) — the #1-weakness lever (35% first-move cutoff vs ~90%). Replace flat 0/1/2 LMR tiers with r≈0.75+ln(d)·ln(i)/2.25 table; behind a flag; gate.
- NNUE: relabel 329k/1M d20 (continuing; definitive large-corpus gate when ~1M, or reuse for d12+d20 combine — data-limited per the -372 calibration).
